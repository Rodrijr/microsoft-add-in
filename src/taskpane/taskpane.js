window.onload = function () {
  // Notifica al taskpane que el iframe se ha cargado, usando el origen correcto
  window.parent.postMessage('iframe_loaded', 'https://iadbdev.service-now.com');
};

window.addEventListener('message', async function (event) {
  // Verifica que el mensaje provenga del origen correcto
  if (event.origin === 'https://rodrijr.github.io' && event.data === 'start_fetch') {
    try {
